...
- Lockdown is not going to last forever and people will eventually be spending less time at home, so the logical next point for this business would be to set up a selection box in the checkout that indicates whether the user wishes to collect in-store, removing the cost of delivery.
...
I used [GitHub](https://www.github.com/) as the code host for this project, and [GitPod](https://www.gitpod.io/) to write it, using just a single branch. stuff about [Heroku](https://www.heroku.com/). The actual data is stored within a [MongoDB](https://www.mongodb.com/) cluster, and four sub-collections (see the Data Architecture section below).
